...
- **라이브·다시보기 광고 차단을 보강**했습니다. 확인된 광고 소스와 중간 광고 항목을 처리하고, 광고에 연동되는 배너를 추가로 숨깁니다. 광고 차단은 실험 기능이며 켜고 저장한 뒤 치지직 탭을 새로고침해야 합니다.
- **광고 차단과 자동 화질을 함께 사용할 때의 충돌**을 수정하고, 배속·시간 이동·화질 조절에서 광고 영상이 본영상으로 선택되지 않도록 보강했습니다.
- **팔로잉 오프라인 채널 숨기기**를 추가했습니다. 기본값은 꺼짐이며, 켜면 고정 여부와 관계없이 사이드바의 오프라인 채널을 숨깁니다.
- 치지직의 현재 **라이트·다크 테마**에 맞춰 검색·댓글·채팅·사이드바 색상을 보완하고, 검색창 포커스 안내 문구의 대비를 개선했습니다.
- **C 자막 단축키와 배속 키의 충돌**을 방지하고, 통나무 보상 버튼 판별과 페이지 이동·요청 취소 시 정리를 보강했습니다.
- 업데이트 알림은 **새로고침 안내**로 간소화하고 튜토리얼 화면과 다시보기 버튼을 제거했습니다.
...
새 버전으로 업데이트하면 확장 아이콘에 `NEW` 배지와 설정 상단의 업데이트 안내가 나타납니다. 열려 있는 치지직 탭에는 상단 검색창 옆에 **새로고침 / 알림 끄기** 버튼을 표시합니다.

**새로고침**을 누르면 현재 버전의 알림을 확인 처리하고 해당 탭만 다시 불러옵니다. 별도의 튜토리얼이나 기능 안내 페이지 없이 바로 시청을 이어갑니다. 확인 상태를 저장하지 못하면 오류를 표시하며 다시 누를 수 있습니다.

**알림 끄기** 또는 설정의 **팝업·알림 → 업데이트 알림**을 끄고 저장하면 이후 업데이트 알림과 `NEW` 배지를 숨깁니다. 필요할 때 같은 설정에서 다시 켤 수 있습니다. 광고 차단 안내 팝업 숨김과 업데이트 알림은 별도 옵션입니다.
...
배속 키 입력칸을 선택한 뒤 영문·숫자·일부 기호 키를 누르면 변경됩니다. Space, `c`/`m`/`f`/`t`, `j`/`k`/`l`, 방향키, 쉼표·마침표처럼 기존 재생 조작과 겹치는 키와 조합키는 지정할 수 없습니다. 기존에 C를 배속 키로 저장했다면 해당 항목을 기본 키로 복원합니다.
