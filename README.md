...
설정의 **팝업·알림 → 업데이트 알림**을 끄고 저장하면 이후 업데이트 알림과 `NEW` 배지를 모두 숨깁니다. 광고 차단 안내와 업데이트 알림은 별도 그룹으로 구분되어 있습니다. 같은 설정에서 다시 켤 수 있습니다. **튜토리얼 다시보기**를 누르면 열려 있는 치지직 탭에서 사이드바·멀티뷰 안내를 첫 단계부터 다시 시작합니다. 활성 치지직 탭을 우선하며, 없으면 최근 사용한 치지직 탭을 선택합니다. 업데이트 알림을 꺼 두었거나 이미 확인했어도 실행할 수 있고, 완료하거나 닫아도 업데이트 알림 설정과 확인 상태는 바꾸지 않습니다. 치지직 탭이 없으면 먼저 열어 달라는 안내를 표시합니다.
