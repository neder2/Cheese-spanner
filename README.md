...
- **동영상 광고 차단(실험적)**과 **배너 광고 숨기기**를 추가했습니다. 두 옵션은 기본 꺼짐이며, 동영상 광고 차단은 켜고 저장한 뒤 치지직 탭을 새로고침해야 합니다.
- **보조 방송 정렬**이 오른쪽·아래쪽 등 모든 보조 영역에 적용되도록 수정하고, 멀티뷰 버튼과 음량 도구의 표시를 다듬었습니다.
- **동시 시청 시간 중복 집계**를 수정하고 시청 기록 저장 위치 안내, 기록 데이터 검증과 월간 방송 달력의 조회·취소·캐시 처리를 보강했습니다.
- **단축키 복구 기능을 제거**했습니다. Space 홀드 배속·고정 배속·시간 이동 단축키는 유지합니다. 광고 자동 건너뛰기는 이번 버전에서 공개하지 않습니다.
...
튜토리얼은 **광고 차단 기능 추가** 한 장으로 구성하며, **광고 차단, 중간 광고 차단, 배너 광고 차단을 지원해요. 아직 실험적인 기능이라 불완전할 수 있어요.**라고 안내합니다. 튜토리얼 안내는 스튜디오 버튼을 거의 덮는 위치에 표시하며, 버튼이 없으면 기존 페이지 상단 위치를 사용합니다. 한 장뿐이므로 이전·다음 화살표는 숨깁니다.

설정의 **팝업·알림 → 업데이트 알림**을 끄고 저장하면 이후 업데이트 알림과 `NEW` 배지를 모두 숨깁니다. 광고 차단 안내와 업데이트 알림은 별도 그룹으로 구분되어 있습니다. 같은 설정에서 다시 켤 수 있습니다. **튜토리얼 다시보기**를 누르면 열려 있는 치지직 탭에서 현재 버전의 안내를 첫 단계부터 다시 시작합니다. 활성 치지직 탭을 우선하며, 없으면 최근 사용한 치지직 탭을 선택합니다. 업데이트 알림을 꺼 두었거나 이미 확인했어도 실행할 수 있고, 완료하거나 닫아도 업데이트 알림 설정과 확인 상태는 바꾸지 않습니다. 치지직 탭이 없으면 먼저 열어 달라는 안내를 표시합니다.

치지직 상단 검색창 오른쪽 여백에 폭 280px의 작은 업데이트 알림이 겹쳐 표시됩니다. 업데이트 완료 문구와 새 기능 안내 문장 바로 옆에 **새로고침 / 알림 끄기** 버튼이 나란히 보입니다. **새로고침**을 누르면 현재 탭에 업데이트를 적용하고 **광고 차단 기능 추가** 안내를 펼칩니다. **알림 끄기**(안내에서 **업데이트 다시 알리지 않음**)는 이번 버전뿐 아니라 앞으로의 업데이트 알림까지 끕니다. 업데이트 당시 열려 있는 치지직 탭에도 알림을 표시하며, 새로고침은 버튼을 누른 탭에서만 실행됩니다.

새 버전으로 업데이트하면 확장 아이콘에 `NEW` 배지와 설정 상단의 업데이트 안내가 나타납니다. **새 기능 안내**에서 우측 상단의 이전·다음 화살표로 짧은 설명을 넘겨 봅니다. 안내에 적힌 옵션 위치로 직접 이동해 기능을 설정해 주세요. 마지막 **확인**을 누르면 알림이 사라지고, **나중에**나 닫기는 미확인 상태를 유지합니다. 치지직 스튜디오 버튼 근처에서 광고 차단 기능 추가와 실험적인 기능이라는 점을 안내합니다. 설정의 **팝업·알림 → 업데이트 알림 → 튜토리얼 다시보기**로 치지직 화면의 안내를 다시 볼 수 있습니다. 안내는 설정을 자동 변경하지 않으며, 확인한 버전은 이 기기에만 저장합니다.
...
- **고정 배속 단축키**와 **시간 이동 단축키**는 각각의 설정을 따릅니다.
...
- **동영상 광고 차단 — 실험적:** 확인된 치지직 광고 표시 응답의 프리롤·미드롤 값을 처리합니다. 켜고 저장한 뒤 치지직 탭을 새로고침해야 합니다. 자동으로 새로고침하지 않습니다. 끄면 이후 응답 처리를 멈추며, 이미 반영한 플레이어 상태는 새로고침하면 복원됩니다. 알 수 없는 응답과 본방송 스트림·화질 정보는 그대로 통과합니다.
- **배너 광고 숨기기:** 홈·라이브·VOD에서 확인한 광고 배너 슬롯만 CSS로 숨깁니다. 기능을 끄면 원래 표시로 돌아갑니다. 이 옵션 자체는 광고 요청을 차단하지 않습니다.

동영상 광고 차단은 실험 기능으로, 치지직 응답이나 플레이어가 바뀌면 광고가 재생될 수 있습니다. 다시보기에서는 확인된 치지직 광고 스케줄의 플레이어 연결도 차단해 중간 광고 준비와 암전 카운트다운이 시작되지 않도록 처리합니다. 적용하려면 치지직 탭을 새로고침해야 하며, 이미 시작된 광고를 즉시 제거하는 기능은 아닙니다. SDK 교체나 광고 서버 전체 차단은 하지 않습니다.
...
| **동영상 광고 차단 — 실험적** (`adVideoEnabled`)            | 확인된 프리롤·미드롤 광고 표시 응답 처리, 저장 후 새로고침  | 꺼짐     | —               |
...
- **`scripting`** — 업데이트 당시 열려 있는 치지직 탭에 패키지에 포함된 업데이트 알림 코드를 실행합니다. 동영상 광고 차단을 켜면 패키지 내부 광고 처리 스크립트를 치지직 문서 시작 시점에 실행하도록 등록하고, 끄면 등록을 해제합니다. 대상은 `https://chzzk.naver.com/*`입니다.
